DECLARE
   CURSOR cur_ret IS
      ...
      AND c_formulario = 'FCP'
      AND n_formulario IN (16609 ,50202, 50203, 50204 ,53061);
   --
   ...
   CURSOR cur_ret IS
      SELECT *
       ...
       WHERE a.aa_factura = 2025
         AND a.t_factura IN ('FCP')
         --and a.e_factura not in 'X'
         AND a.o_factura IN 
      (16609 ,50202, 50203, 50204 ,53061);
   --
BEGIN
   --
   FOR i IN cur_ret LOOP
...
DECLARE
   CURSOR cur_ret IS
      SELECT * FROM slu.tasiecab
      WHERE aa_ejercicio_frm = 2025
      AND c_formulario = 'FBS'
      AND n_formulario IN (9258, 9280);
   --
   l_rutina    ing_tregistro_error.x_rutina%TYPE := 'SLU.SPAT_CIERRE_CONTABLE.borrar_cierre';
BEGIN
   --
...
DECLARE
   CURSOR cur_ret IS
      SELECT * FROM slu.tasiecab
      WHERE aa_ejercicio_frm = 2025
      AND c_formulario = 'FFC'
      AND n_formulario IN (37937, 40230);
   --
   l_rutina    ing_tregistro_error.x_rutina%TYPE := 'SLU.SPAT_CIERRE_CONTABLE.borrar_cierre';
BEGIN
   --
